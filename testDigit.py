...
equal_path = "C:\\Users\\sabik\\PycharmProjects\\DeepLearningBasics\\"
equal_result_path = "C:\\Users\\sabik\\PycharmProjects\\DeepLearningBasics\\AnResult"
equal_boxed_path = "C:\\Users\\sabik\\PycharmProjects\\DeepLearningBasics\\AnOutput"
...
def isDot(boundingBox):
    ...
    area = (yh - y) * (xw - x)
    return area < 200 and 0.5 < (xw - x) / (yh - y) < 2 and abs(xw - x) < 20 and abs(
        yh - y) < 20  # 100 is migical number
...
def isLetterI(boundingBox, boundingBox1):
    (x, y), (xw, yh) = boundingBox
    (x1, y1), (xw1, yh1) = boundingBox1
    return (((isDot(boundingBox) and isVerticalBar(boundingBox1)) or (
                isDot(boundingBox1) and isVerticalBar(boundingBox)))
...
def isEquationMark(boundingBox, boundingBox1):
    (x, y), (xw, yh) = boundingBox
    (x1, y1), (xw1, yh1) = boundingBox1
    return isHorizontalBar(boundingBox) and isHorizontalBar(boundingBox1) and abs(x1 - x) < 20 and abs(
        xw1 - xw) < 20  # 20 is a migical number
...
def isDots(boundingBox, boundingBox1, boundingBox2):
    ...
    cenY1 = y1 + (yh1 - y1) / 2
    cenY2 = y2 + (yh2 - y2) / 2
    return (isDot(boundingBox) and isDot(boundingBox1) and isDot(boundingBox2) and max(cenY, cenY1, cenY2) - min(cenY,
                                                                                                                 cenY1,
                                                                                                                 cenY2) < 50)  # 30 is a migical number
...
def isFraction(boundingBox, boundingBox1, boundingBox2):
    ...
    cenX1 = x1 + (xw1 - x1) / 2
    cenX2 = x2 + (xw2 - x2) / 2
    case1 = not isDot(boundingBox) and not isDot(boundingBox1) and isHorizontalBar(boundingBox2) and (
                y < y2 < yh1 or y1 < y2 < yh)
    case2 = not isDot(boundingBox2) and not isDot(boundingBox) and isHorizontalBar(boundingBox1) and (
                y2 < y1 < yh or y < y1 < yh2)
    case3 = not isDot(boundingBox1) and not isDot(boundingBox2) and isHorizontalBar(boundingBox) and (
                y1 < y < yh2 or y2 < y < yh1)
...
def saveImages(im, boxes, im_name):
    ...
    for box in boxes:
        ...
        if w < 25 and h < 25 and float(w) / h < 1.5 and float(h) / w < 1.5:
            symbolImage.save(
                equal_boxed_path + im_name + "_" + "dot" + "_" + str(y) + "_" + str(yh) + "_" + str(x) + "_" + str(
                    xw) + ".png")
        else:
            # save rectangled element
            symbolImage.save(
                equal_boxed_path + im_name + "_" + str(num) + "_" + str(y) + "_" + str(yh) + "_" + str(x) + "_" + str(
                    xw) + ".png")
